...
parser.add_argument('--weights', nargs='+', type=str, default='weights/yolov5s.tflite', help='model.pt path(s)')
parser.add_argument('--source', type=str, default='static/uploads', help='source')  # file/folder, 0 for webcam  #data/images
...
parser.add_argument('--project', default='static/detect', help='save results to project/name')
...
def detect(save_img=False, opt=opt):
    ...
    #Uncommet     save_dir = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
